...
describe('Response', () => {
  describe('isFinished', () => {
    ...
    it('returns false for an unfinished tree', () => {
      expect(makeResponse('incident', unfinishedAnswerEndingInSingleQuestion).isFinished).to.be
        .false
    })
  ...
  describe('highestIncidentLevel', () => {
    it('returns ACCIDENT for an accident tree', () => {
      expect(makeResponse('incident', accidentAnswer).highestIncidentLevel).to.eql(
        IncidentLevel.ACCIDENT
      )
    })

    it('returns SERIOUS INCIDENT for a serious incident tree', () => {
      expect(makeResponse('incident', seriousIncidentAnswer).highestIncidentLevel).to.eql(
        IncidentLevel.SERIOUS_INCIDENT
      )
    })

    it('returns INCIDENT for an incident tree', () => {
      expect(makeResponse('incident', incidentAnswer).highestIncidentLevel).to.eql(
  ...
  describe('highestPossibleIncidentLevel', () => {
    ...
    it('returns ACCIDENT if accident is still possible', () => {
      ...
      expect(makeResponse('incident', answer).highestPossibleIncidentLevel).to.eql(
        IncidentLevel.ACCIDENT
      )
    })

    it('returns SERIOUS INCIDENT if accident is no longer possible', () => {
      expect(makeResponse('incident', possibleSeriousIncident).highestPossibleIncidentLevel).to.eql(
        IncidentLevel.SERIOUS_INCIDENT
      )
    })

    it('returns INCIDENT if nothing else possible', () => {
      expect(makeResponse('incident', incidentAnswer).highestPossibleIncidentLevel).to.eql(
        IncidentLevel.INCIDENT
      )
    })
  ...
  describe('nextQuestion', () => {
    it('returns the next Question', () => {
      ...
      expect(isQuestion(prompt!.question)).to.be.true
  ...
  describe('contributingRegulations', () => {
    it('returns a list of regulations for options the user chose', () => {
      expect(makeResponse('incident', accidentAnswer).contributingRegulations).to.eql(
        new Set(['830.2'])
      )
      expect(makeResponse('incident', seriousIncidentAnswer).contributingRegulations).to.eql(
        new Set(['830.5'])
      )
      expect(makeResponse('incident', incidentAnswer).contributingRegulations).to.eql(
        new Set(['830.5'])
      )
    })
  ...
  describe('flags', () => {
    it('returns the list of flags that were set', () => {
      ...
      expect(makeResponse('profile', answer).flags).to.eql(
